...
class Datapath(val conf: CoreConfig) extends Module {
  ...
  val mem_stall = !io.icache.resp.valid || !io.dcache.resp.valid
  val pc = RegInit(Consts.PC_START.U(conf.xlen.W) - 4.U(conf.xlen.W))
  
  // define load-use hazard
  val load_use_hazard = WireInit(false.B)
  val stall = mem_stall || load_use_hazard
  ...
  val inst =
    Mux(
      started || io.ctrl.inst_kill || (io.ctrl.pc_sel === PC_ALU) || de_reg.taken || brCond.io.taken
        || csr.io.expt || de_reg.pc_sel === PC_ALU || de_reg.pc_sel === PC_EPC || em_reg.pc_sel === PC_EPC
        || load_use_hazard, // Insert NOP on load-use hazard
      Instructions.NOP,
  ...
  load_use_hazard := (de_reg.ld_type =/= LD_XXX) &&
    ((de_rd_addr === rs1_addr && rs1_addr =/= 0.U) ||
      (de_rd_addr === rs2_addr && rs2_addr =/= 0.U))
  ...
  // bypass ex/de - we already defined de_rd_addr for load-use hazard detection
